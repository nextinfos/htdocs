<?php
	...
	switch (substr($code, 0, 3)){
		case "STD":	$type = "student";	break;
		case "INS":	$type = "instructor";	break;
		default: $type = "student"; break;
	}
	$barcodeType= 8;/*8=CODE39;20=CODE128*/
	$barcodeChar = substr($code, 0, 3);
?>
<!DOCTYPE html>
<html>
	<head>
		<meta charset="UTF-8">
		<title>ระบบออกบัตรนักเรียน</title>
...
/* 				-webkit-fill-color: #FFFFFF; */
	...
	<body>
		...
		<table>
		<?php
			$strSQL = sprintf("SELECT * FROM `%s` WHERE `%sID` = '%s';",$type,$type,strtolower($id));
			$objQuery = mysql_query($strSQL);
			if($objQuery){
			$c=1;
			while($row = mysql_fetch_array($objQuery)){
				$studentID=$row[$type.'ID'];
				$studentFName=$row['firstName'];
				$studentLName=$row['lastName'];
				$studentPhoto = 'images/'.$type.'s/'.$studentID.'.jpg';
